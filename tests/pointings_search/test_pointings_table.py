...
def test_clear_rows():
    ...
    data.clear_rows([1, 3])
    assert len(data.pointings) == 3
    assert len(data.pointings.columns) == 3
    assert data.pointings["ra"][1] == 45.0
    assert data.pointings["dec"][1] == 0.0
    assert data.pointings["obstime"][1] == 2.0

    # Clear everything
    data.clear_rows()
    assert len(data.pointings) == 0
    assert len(data.pointings.columns) == 3

    # Its okay to call clear on an empty table.
    data.clear_rows()
...
def test_copy():
    ...
    # Change the copy.
    data2.pointings["ra"][1] = 125.0
    data2.clear_rows([3])
    assert len(data2.pointings) == 4
    assert len(data2.pointings.columns) == 4
    assert data2.pointings["ra"][1] == 125.0

    # Check that the original data is unchanged.
    assert len(data.pointings) == 5
    assert len(data.pointings.columns) == 4
    assert data.pointings["ra"][1] == 90.0
